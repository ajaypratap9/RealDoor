...
4. Never state or imply whether this person is eligible for any program.
...
export async function POST(request) {
  ...
  try {
    ...
    const { imageBase64Array, mimeType } = body;
    ...
    const userContent = [
      { type: "text", text: "Extract the allowlisted fields from this document image. The document may have multiple pages." }
    ];

    imageBase64Array.forEach(img => {
      userContent.push({ type: "image_url", image_url: { url: `data:${mimeType};base64,${img}` } });
    });
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: "Groq extraction failed", details: text }, { status: 500 });
    }
    ...
    // Defensive check
    const allowedKeys = [
      "employer_or_agency_name", "gross_pay_amount",
      "pay_period_or_issue_date", "benefit_amount", "document_date",
    ];
    
    if (parsed.fields) {
      for (const key of Object.keys(parsed.fields)) {
        if (!allowedKeys.includes(key)) delete parsed.fields[key];
      }
    }
